...
import numpy as np
from leafsyn.tasks import CutPastePatchBlender,\
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, source, preprocess, k_shot=-1, **kwargs):
        super().__init__()
        self.args = args
        self.source = source
        self.k_shot = k_shot
    ...
    def get_image_data(self):
        data_to_iterate = []
        json_file_path = os.path.join(self.source, 'samples', "train.json")
        try:
            with open(json_file_path, "r") as f_r:
                for line in f_r:
                    try:
    ...
    def load_anomaly_syn(self):
        ...
        for task_name in self.args.anomaly_tasks.keys():
            if task_name == 'CutpasteTask':
                support_images = [self.read_image(os.path.join(self.source, 'images', data['filename']))
...
class PlantTestDataset(torch.utils.data.Dataset):
    ...
    def get_image_data(self):
        data_to_iterate = []
        json_file_path = os.path.join(self.source, 'samples', "test.json")
        try:
            with open(json_file_path, "r") as f_r:
                for line in f_r:
                    try:
