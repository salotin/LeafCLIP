...
from datasets.dataset import TrainDataset, PlantTestDataset
import pprint
...
def evaluate_metrics(y_true, y_pred):
    ...
    f1 = f1_score(y_true, y_pred, average='macro')  # 使用宏平均F1
    return mAP, f1
...
def main(args):
    ...
    for test_dataset_name in args.config.test_datasets:
        if test_dataset_name == 'plant':
            ...
            raise NotImplementedError("dataset must in ['plant'] ")
    ...
    logger.info(f"train data ({args.config.train_dataset}) len {len(train_dataset)}")

    for test_dataset_name in test_dataloaders:
        logger.info(f"test data ({test_dataset_name}) len {len(test_dataloaders[test_dataset_name].dataset)}")

    for task_name in args.config.anomaly_tasks:
        logger.info(f"anomaly syn task is {task_name}, sampling probability is {args.config.anomaly_tasks[task_name]}")
        ...
        if (epoch + 1) % args.config.val_freq_epoch == 0:
            ...
            for test_dataset_name in results:
                if best_record[test_dataset_name] is None:
                    if test_dataset_name == 'plant':
                        best_record[test_dataset_name] = [results[test_dataset_name]["images-auroc"],
                ...
                logger.info(f"({test_dataset_name}): Epoch: {epoch+1}, "
                            f"images auroc: {results[test_dataset_name]['images-auroc']:.4f}, "
                            f"pixel_auroc: {results[test_dataset_name]['pixel-auroc']:.4f},")

            if save_flag:
                logger.info(f"save checkpoints in epoch: {epoch+1}")
                torch.save({
                    "adapter_state_dict": adapter.state_dict(),
                    "prompt_state_dict": prompt_maker.prompt_learner.state_dict(),
                }, os.path.join(args.config.save_root, f'checkpoints_{epoch + 1}.pkl'))

def train_one_epoch(args, train_dataloader, optimizer, epoch, start_iter, logger, clip_model, necker, adapter, prompt_maker, map_maker):
    ...
    for i, input in enumerate(train_dataloader):
        ...
        images = input['images'].to(clip_model.device)
        ...
        if (curr_step + 1) % args.config.print_freq_step == 0:
            logger.info(f"Epoch: [{epoch + 1}/{args.config.epoch}]\t"
                        f"Iter: [{curr_step + 1}/{len(train_dataloader) * args.config.epoch}]\t"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", type=str, default='config/plant.yaml', help="model configs")
    parser.add_argument("--k_shot", type=int, default=16, help="normal images number")
